...
def sent_anly_prediction():
    if request.method=='POST':
        ...
        class1 = np.argmax(prediction[0][0])

        if probability <0.5/100:
            sentiment = "NEGATIVE 🙁🙁🙁" 
        else:
            sentiment = 'POSITIVE 😊😊😊'
            
    return render_template('result.html', text=text, sentiment=sentiment, probability=probability*100)
